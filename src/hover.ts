...
class HoverProvider implements vscode.HoverProvider {
  public provideHover(
    ...
    const stageCommandUri = vscode.Uri.parse(
      `command:assistiveTools.convertEnum?${encodeURIComponent(
        JSON.stringify(args)
      )}`
    );
    const markdownString = new vscode.MarkdownString(
      `[生成枚举转换](${stageCommandUri})`
    );
 ...
 * @returns
 */
function parseEnumObjects(
  document: vscode.TextDocument,
  enumName: EnumNameMap
): EnumParseResult {
  ...
  if (
    sourceCode.match(
      new RegExp(`${enumName.mapName}|${enumName.optionName}`, "gm")
    )
  ) {
...
function showAutoClosingInformationMessage(
  message: string,
  timeout = 2000
): void {
