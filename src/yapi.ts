 ...
 */
export default function registerYapiInterfaceGenerator(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand("assistiveTools.generateYapiInterface", async () => {
      ...
      const urlInput = await vscode.window.showInputBox({
        placeHolder: "例如：https://yapi.example.com/project/25/interface/api/80173",
      ...
      if (!interfaceId) {
        void vscode.window.showErrorMessage("未从地址中解析到接口 ID，请检查链接是否为接口详情页")
        return
      }

      const apiUrl = `${parsed.origin}/api/interface/get?id=${interfaceId}`
      const detail = await fetchInterfaceDetail(apiUrl, parsed.href, token, interfaceId)
      if (!detail) {
        return
      }
      ...
      if (!snippet) {
        void vscode.window.showWarningMessage("未获取到可生成的请求或响应信息")
        return
      }
  ...
  try {
    return await vscode.window.withProgress(
      ...
      async () => {
        ...
        if (!response.ok) {
          throw new Error(`请求失败，HTTP 状态码 ${response.status}: ${response.statusText}`)
        }
...
function buildInterfaceSnippet(detail: YapiInterfaceDetail, interfaceId: string): string {
  const title = detail.title || `接口 ${interfaceId}`
  const header = `// ${title} (${detail.method ?? "GET"} ${detail.path ?? ""})`
  const baseName = toPascalCase(title) || `Interface${interfaceId.replace(/[^0-9A-Za-z]/g, "")}`
...
function buildRequestBody(detail: YapiInterfaceDetail, name: string): string | undefined {
  const schema = parseSchema(detail.req_body_is_json_schema, detail.req_body_other)
  if (schema) {
...
function buildResponseInterface(detail: YapiInterfaceDetail, name: string): string | undefined {
  if (!detail.res_body) {
    return undefined
  }
...
function assembleInterfacesFromSchema(schema: JsonSchema, name: string): string {
...
function inferSchemaFromSample(sample: any): JsonSchema {
  if (Array.isArray(sample)) {
    return {
      type: "array",
      items: sample.length > 0 ? inferSchemaFromSample(sample[0]) : { type: "any" },
    }
  }
